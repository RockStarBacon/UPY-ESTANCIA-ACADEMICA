...
# Conexión PostgreSQL
engine = create_engine("postgresql+psycopg2://postgres:postgres@localhost:5432/prueba_pipeline")
...
for estacion in estaciones:
    ...
    while fecha_actual <= fecha_fin:
        ...
        try:
            ...
            if not tabla:
                print(f"No hay tabla en {url}")
                fecha_actual += timedelta(days=1)
                continue

            rows = tabla.find('tbody').find_all('tr')
            if not rows:
                print(f"No hay filas en {url}")
                fecha_actual += timedelta(days=1)
                continue
            ...
            df = pd.DataFrame({
                'datetime_medicion': [
                    datetime.combine(fecha_actual.date(), datetime.strptime(t, '%I:%M %p').time())
                    for t in tiempo
                ],
            ...
            # Leer datos existentes en la base de datos
            ...
            if not df_nuevo.empty:
                ...
                print(f"✅ Insertados {len(df_nuevo)} registros nuevos de {estacion} para {fecha_str}")
            else:
                print(f"⏩ No hay datos nuevos para {estacion} en {fecha_str}")

        except Exception as e:
            print(f"❌ Error procesando {estacion} en {fecha_str}: {e}")
